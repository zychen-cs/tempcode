...
def calculation_parallel(magcount=1, use_kf=0, use_wrist=False):
    ...
    data = pd.read_csv("/home/czy/windows_disk/Users/26911/Documents/linux/trackingdata/1210LMsensor1_1.csv")
    ...
    for j in range (0,len(data)):
        ...
        if not worklist.empty():
                ...
                #           np.sin(ang_convert(resulti[7]))*np.sin(ang_convert(resulti[8])), np.cos(ang_convert(resulti[7]))])
                    ...
                    # print(resultslist)
                    if(len(resultslist)==len(data)):
                        print("Output csv")
                        test = pd.DataFrame(columns=name,data=resultslist)
                        test.to_csv("/home/czy/桌面/magx-main1/1210LMresults1.csv")
                        print("Exited")
                        countnum=countnum+1
                        resultslist=[]
