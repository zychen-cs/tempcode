...
cali_path = '/home/czy/桌面/magx-main1/1228_cali1.csv'
name = ['Time Stamp', 'x',
        'y', 'z', 'runtime']
...
def calculation_parallel(magcount=1, use_kf=0, use_wrist=False):
    ...
    while True:
        if not worklist.empty():
                ...
                mapping_net_model.load_state_dict(torch.load('/home/czy/桌面/magx-main1/Codes/optimization/1230mapping_net_model.pth',map_location=torch.device('cpu')))

                mlp_model = MLP().to(device)
                mlp_model.load_state_dict(torch.load('/home/czy/桌面/magx-main1/Codes/optimization/1230_differential_mlp_model_diff.pth',map_location=torch.device('cpu')))
            ...
            #     model.eval()
                ...
                if(len(resultslist)==300):
                     ...
                     test.to_csv("/home/czy/windows_disk/Users/26911/Documents/linux/trackingdata/1226MLP1_"+str(countnum)+".csv")
                     print("Exited")
                     countnum=countnum+1
                     resultslist=[]
                # current = [datetime.datetime.now()]
