    ...
    def build_dict(self):
        ...
        if old:
            ...
            for i in range (0,len(self.df)-1):
                result=[]
                for j in range(24):
                    result.append(self.df.iloc[i,j])
                # print(result)
                readings.append(result)
            readings = np.array(readings)
            # print(readings)
            self.raw_readings = readings
            self.readings = self.raw_readings.copy()
...
class Realtime_data:
    ...
    def __init__(self, csv_path):
        ...
        # self.build_dict()
        # self.offset = np.array([-6.79864, -33.4728, 2.20068]) - np.array([15.15802119, -0.1380165, 0.06874539]) + np.array([12, 0, 0])
...
#position的修改地方
        ...
        new_positions[:, 1] = self.positions[:, 2]
        ...
        # new_direction[:, 1] = -self.directions[:, 1]
        # new_direction[:, 2] = self.directions[:, 0]
...
#position的修改地方
        ...
        new_direction = self.directions.copy()
        new_direction[:, 0] = -self.directions[:, 0]
        new_direction[:, 1] = self.directions[:, 2]
        new_direction[:, 2] = self.directions[:, 1]
        # new_direction[:, 0] = self.directions[:, 2]
        # new_direction[:, 1] = -self.directions[:, 1]get_gt
        # new_direction[:, 2] = self.directions[:, 0]
