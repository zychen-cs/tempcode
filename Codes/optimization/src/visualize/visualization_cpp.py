...
from ..preprocess import Reading_Data, LM_data, LM_data_2mag,Realtime_data
...
def show_track_1mag_csv_cpp(reading_path, cali_path, gt_path, pSensor, My_M, use_kalman=False):
    ...
    # reading_data.tstamps = reading_data.tstamps + 16 * 60 * 60
    lm_data = LM_data(gt_path)
    # set the origin of the gt
    # lm_data.offset = np.array([-6.79864, -33.4728, 2.20068]) - np.array([15.15802119, -0.1380165, 0.06874539])
    # lm_data.offset = np.array([-6.79864, -33.4728, 2.20068]) - np.array([1.1699,-14.0412,-  ])
    result_parameter = []
# 7.34331, 139.512, -11.699)
    color = ['r', 'b', 'g', 'y', 'm']
    for index, M in enumerate(M_choice):
        ...
        if use_kalman:
            ...
            params = np.array([40 / np.sqrt(2) * 1e-6, 40 / np.sqrt(2) * 1e-6, 0, np.log(
                My_M), 1e-2 * starting_point[0], 1e-2 * starting_point[1], 1e-2 * starting_point[2], 0, 0])
            params = np.array([40 / np.sqrt(2) * 1e-6, 40 / np.sqrt(2) * 1e-6, 0, np.log(
                My_M), 1e-2 * (-0.8), 1e-2 * (4.5), 1e-2 * (9), np.pi/2, np.pi])
            #6,3.5,3.7
            #5,3.5,3.7
...
# np.pi/2, np.pi
    ...
    # lm_data.offset = pred_position[0]
    print("position: ", pred_position)
    # sensor position
    ts_label = reading_data.tstamps[cut]
    index_label = 0
    # print(ts_label)
    # print(lm_data.tstamps[-1])
    # while lm_data.tstamps[index_label] < ts_label:
    #     index_label += 1
    # lm_data.offset = lm_data.positions[index_label] - pred_position[cut]

    lm_data.offset = starting_point - pred_position[0]
...
def show_track_1mag_csv_cpp_modify(reading_path, cali_path, gt_path, pSensor, My_M, use_kalman=False):
    ...
    # reading_data.tstamps = reading_data.tstamps + 16 * 60 * 60
    lm_data = LM_data(gt_path)
    pred_data = pd.read_csv(reading_path)
    pred_position = []
    directions = []
    print(pred_data)
    tstamps=[]
    tstamps=pred_data['Time Stamp'].map(time_mapping).to_numpy()
    for i in range(0,len(pred_data)):
        ...
        direction = [
                ...
                np.array([np.sin(pred_data['theta'][i])*np.cos(pred_data['phy'][i]),
                          np.sin(pred_data['theta'][i])*np.sin(pred_data['phy'][i]), np.cos(pred_data['theta'][i])]),
            ]
        directions.append(direction)
        pred_position.append(x0)
        pred_position.append(y0)
...
# 7.34331, 139.512, -11.699)
    ...
    # lm_data.offset = lm_data.positions[index_label] - pred_position[cut]
    # print(tstamps[0])
    lm_data.offset = lm_data.get_gt(tstamps[0]) - pred_position[0]
    ...
    for i in range(pred_position.shape[0]):
        ...
        # print(gt_direction)
        # print(directions[i])
        loss2 = np.arccos(np.dot(gt_direction, directions[i][0]))
    ...
    np.savez(os.path.join(result_path, file_name),
             gt=gt_route,
             result=pred_position, gt_ang=gt_directions, result_ang=directions)
