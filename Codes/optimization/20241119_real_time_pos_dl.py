...
class CustomCNN(nn.Module):
    def __init__(self):
        ...
        # Reshape 1D input of size (24) to (1, 6, 4) for 2D convolution
        self.initial_conv = nn.Conv2d(1, 64, kernel_size=1)  # 1x1 Conv, 64 filters
        self.conv1 = nn.Conv2d(64, 64, kernel_size=3, padding=1)  # 3x3 Conv, 64 filters
        self.bn1 = nn.BatchNorm2d(64)

        # Second block with increased filters
        self.conv2 = nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=1)  # Downsampling by stride 2
        self.bn2 = nn.BatchNorm2d(128)
        self.conv2_adjust = nn.Conv2d(64, 128, kernel_size=1, stride=2)  # 1x1 Conv to match dimensions
        self.bn2_adjust = nn.BatchNorm2d(128)

        # Third block with more filters
        self.conv3 = nn.Conv2d(128, 256, kernel_size=3, stride=2, padding=1)  # Further downsampling
        self.bn3 = nn.BatchNorm2d(256)
        self.conv3_adjust = nn.Conv2d(128, 256, kernel_size=1, stride=2)  # 1x1 Conv for dimension match
        self.bn3_adjust = nn.BatchNorm2d(256)

        # Global average pooling
        self.avg_pool = nn.AdaptiveAvgPool2d((1, 1))  # Global pooling to 1x1

        # Fully connected layer for final 5-dimensional output
        self.fc = nn.Linear(256, 5)

    def forward(self, x):
        # Reshape the input tensor from (batch_size, 24) to (batch_size, 1, 6, 4)
        x = x.view(-1, 1, 6, 4)
        ...
        residual = F.relu(self.bn2_adjust(self.conv2_adjust(residual)))  # Adjusted residual connection
        x = F.relu(self.bn2(self.conv2(x)))
        x += residual  # Add residual

        # Third block with further downsampling
        residual = F.relu(self.bn3_adjust(self.conv3_adjust(residual)))  # Adjusted residual connection
    ...
    while True:
        if not worklist.empty():
            ...
            #     model.load_state_dict(torch.load('/home/czy/桌面/magx-main1/Codes/optimization/full_data_mlp_model.pth', map_location=torch.device('cpu')))
                model.load_state_dict(torch.load('/home/czy/桌面/magx-main1/Codes/optimization/1115CNN_model_balanced.pth', map_location=torch.device('cpu')))
                model.eval()
                ...
                # 使用模型进行预测
                with torch.no_grad():
                    y_pred_tensor = model(datai_tensor)
                ...
                if(len(resultslist)==1000):
                     ...
                     test.to_csv("/home/czy/windows_disk/Users/26911/Documents/linux/trackingdata/1124CNN1_"+str(countnum)+".csv")
                     print("Exited")
                     countnum=countnum+1
                     resultslist=[]
                # current = [datetime.datetime.now()]
...
# Updated show_mag_2d function
async def show_mag_2d(magcount=1):
    global results
    myresults = np.array([[0, 0]])  # Only keep x and y data

    fig, ax = plt.subplots(figsize=(8, 8))  # Create 2D plot
    ax.set_xlabel('x(cm)')
    ax.set_ylabel('y(cm)')
    ax.set_xlim([-5, 5])  # Set the range
    ax.set_ylim([-15, 0])  # Set the range

    # Initialize the magnet path plot
    (magnet_pos,) = ax.plot([], [], linewidth=3, animated=True)

    plt.show(block=False)
    plt.draw()  # Perform initial draw to cache the renderer
    ...
    while True:
        ...
        # Update myresults from the queue
        try:
            if not results.empty():
                myresult = results.get_nowait()
                # Only get x and y data
                myresults = np.concatenate([myresults, np.array(myresult[:2]).reshape(1, -1)])
        except Empty:
            pass  # If queue is empty, just continue

        if myresults.shape[0] > 30:  # Limit maximum historical points
        ...
        # Update magnet path
        magnet_pos.set_xdata(x)
        magnet_pos.set_ydata(y)

        # Redraw the updated path
        ax.draw_artist(magnet_pos)
        fig.canvas.blit(fig.bbox)
        
        # Pause to allow proper display
        plt.pause(0.1)  # Increase pause time to ensure smooth refresh
        await asyncio.sleep(0.01)  # Ensure the event loop remains responsive
...
if __name__ == '__main__':
    ...
    if True:
        ...
        still_data = Reading_Data("/home/czy/桌面/magx-main1/1119_I2C_8sensor_441_G.csv")
