...
class MLP(nn.Module):
    def __init__(self, total_epochs=100):
        ...
        # 动态 Dropout
        self.dynamic_dropout = DynamicDropout(max_dropout=0.3, min_dropout=0.1, total_epochs=total_epochs)

        # 残差连接
        self.residual_fc1 = nn.Linear(256, 256)
    ...
    def forward(self, x, current_epoch=None):
        ...
        x2 = F.leaky_relu(self.bn2(self.fc2(x1)) + residual1, negative_slope=0.01)

        # 第三层 + 残差连接
        residual2 = self.residual_fc2(x2)
        x3 = F.leaky_relu(self.bn3(self.fc3(x2)) + residual2, negative_slope=0.01)

        # 第四层 + 残差连接
        residual3 = self.residual_fc3(x3)
        x4 = F.leaky_relu(self.bn4(self.fc4(x3)) + residual3, negative_slope=0.01)

        # 第五层
        x5 = F.leaky_relu(self.bn5(self.fc5(x4)), negative_slope=0.01)

        # 第六层
        x6 = self.dynamic_dropout(F.leaky_relu(self.bn6(self.fc6(x5)), negative_slope=0.01), current_epoch)
...
if __name__ == '__main__':
    ...
    if True:
        ...
        still_data = Reading_Data("/home/czy/桌面/magx-main1/1121_I2C_8sensor_441_G.csv")
