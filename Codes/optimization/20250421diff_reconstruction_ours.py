...
def calculation_parallel(magcount=1, use_kf=0, use_wrist=False):
    ...
    model.load_state_dict(torch.load("/home/czy/桌面/magx-main1/Codes/optimization/ours.pth", map_location=torch.device('cpu')))  # 加载权重
        ...
        if not worklist.empty():
            ...
            #     model.eval()
                ...
                if(len(resultslist)==300):
                     ...
                     test.to_csv("/home/czy/windows_disk/Users/26911/Documents/linux/trackingdata/0605data_our6_"+str(countnum)+".csv")
                     print("Exited")
                     countnum=countnum+1
                     resultslist=[]
                # current = [datetime.datetime.now()]
