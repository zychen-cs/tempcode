...
# params = np.array([40 / np.sqrt(2) * 1e-6, 40 / np.sqrt(2) * 1e-6,
#                    0, np.log(0.46), 1e-2 * (5), 1e-2 * (5), 1e-2 * (2), 0, 0])
#                 #    np.pi/2, np.pi
# params = np.array([40 / np.sqrt(2) * 1e-6, 40 / np.sqrt(2) * 1e-6,
#                    0, np.log(0.25), 1e-2 * (4), 1e-2 * (2), 1e-2 * (-0.17), np.pi, 0])
# params = np.array([40 / np.sqrt(2) * 1e-6, 40 / np.sqrt(2) * 1e-6,
#                    0, np.log(0.25), 1e-2 * (0), 1e-2 * (-5), 1e-2 * (-0.17), 0, 0])

#0.46
#0.2
#1.3
params = np.array([np.log(0.46), 1e-2 * (0), 1e-2 * (-6.5), 1e-2 * (0.7), 0, 0])
...
# cali_path = '/home/czy/桌面/magx-main1/0625cali.csv'
# cali_path = '/home/czy/桌面/magx-main1/1030_I2C_8sensor_441_1.csv'
name = ['Time Stamp', 'x',
        'y', 'z', 'theta', 'phy','runtime']
...
def calculation_parallel(magcount=1, use_kf=0, use_wrist=False):
    ...
    while True:
        if not worklist.empty():
            ...
            if magcount == 1:
                if np.max(np.abs(myparams1[1:4])) > 1:
                    myparams1 = params
                ...
                if(len(resultslist)==300):
                    ...
                    #  test.to_csv("/home/czy/windows_disk/Users/26911/Documents/linux/trackingdata/0707LM_mag88_pos65_motion2_"+str(countnum)+".csv")
                     print("Exited")
                     countnum=countnum+1
                     resultslist=[]
...
def notification_handler(sender, data):
    ...
    if(len(resultslist1)==150):
        print("Output csv")
        test = pd.DataFrame(columns=name1,data=resultslist1)
        # test.to_csv("/home/czy/windows_disk/Users/26911/Documents/linux/trackingdata/0706sensorScissors_"+str(countnum1)+".csv")
        print("Exited")
        countnum1=countnum1+1
        resultslist1=[]
