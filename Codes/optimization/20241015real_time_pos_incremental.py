        ...
        if not worklist.empty():
            ...
            if magcount == 1:
                    ...
                    if(len(resultslist)==1000):
                        ...
                        test.to_csv("/home/czy/windows_disk/Users/26911/Documents/linux/trackingdata/1020test1_"+str(countnum)+".csv")
                        print("Exited")
                        countnum=countnum+1
                        resultslist=[]
...
def notification_handler(sender, data):
    ...
    # if calibration_queue.qsize() == calibration_maxlen and calibrationflag== False:
       ...
       # 加载模型和标准化器
        ...
        # 对每一列计算特征
        features_list = []
        for i in range(Y_new_pred_rescaled.shape[1]):
            column_data = Y_new_pred_rescaled[:, i]  # 选择第i列
            features = calculate_features(column_data)
            features_list.append(list(features.values()))  # 将特征值以列表形式添加

        # 转换为 DataFrame
        features_df = pd.DataFrame(features_list, columns=['f1', 'f2', 'f3', 'f4', 'f5'])

        # print("提取的特征数组：", features_df)

        # 现在可以对 features_df 进行标准化
        scaled_features = G_scalerX.transform(features_df)

        # 使用模型进行预测
        scale = G_scalerXmodel.predict(scaled_features)
