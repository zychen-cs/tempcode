...
params = np.array([40 / np.sqrt(2) * 1e-6, 40 / np.sqrt(2) * 1e-6,
                   0, np.log(0.46), 1e-2 * (0), 1e-2 * (-5), 1e-2 * (-0.17), 0, 0,35.0])
...
# cali_path = '/home/czy/桌面/magx-main1/1202_v1.csv'
    ...
    while True:
        if not worklist.empty():
            ...
            if magcount == 1:
                ...
                if(len(resultslist)==500):
                     print("Output csv")
                     test = pd.DataFrame(columns=name,data=resultslist)
                     test.to_csv("/home/czy/windows_disk/Users/26911/Documents/linux/trackingdata/1210LM3_"+str(countnum)+".csv")
                     print("Exited")
                     countnum=countnum+1
                     resultslist=[]
...
def notification_handler(sender, data):
    ...
    if(len(resultslist1)==500):
        print("Output csv")
        test = pd.DataFrame(columns=name1,data=resultslist1)
        test.to_csv("/home/czy/windows_disk/Users/26911/Documents/linux/trackingdata/1210LMsensor3_"+str(countnum1)+".csv")
        print("Exited")
        countnum1=countnum1+1
        resultslist1=[]
