...
cali_path = '/home/czy/桌面/magx-main1/1225_cali1.csv'
name = ['Time Stamp', 'x',
        'y', 'z', 'runtime']
...
class MappingNet(nn.Module):
    def __init__(self, input_dim=84, hidden_dim=256):  # 可以根据需要调整 hidden_dim
...
# 各个轴的标准差（从你的数据中获得）
std_values = {
    'sensor_1_Bx': 0.485485, 'sensor_1_By': 0.670947, 'sensor_1_Bz': 0.648482,
    'sensor_2_Bx': 0.511365, 'sensor_2_By': 0.714232, 'sensor_2_Bz': 0.643068,
    'sensor_3_Bx': 0.536170, 'sensor_3_By': 0.865630, 'sensor_3_Bz': 0.675086,
    'sensor_4_Bx': 0.484117, 'sensor_4_By': 0.812600, 'sensor_4_Bz': 0.703613,
    'sensor_5_Bx': 0.491070, 'sensor_5_By': 0.556361, 'sensor_5_Bz': 0.798969,
    'sensor_6_Bx': 0.475935, 'sensor_6_By': 0.470903, 'sensor_6_Bz': 0.689016,
    'sensor_7_Bx': 0.496293, 'sensor_7_By': 0.623592, 'sensor_7_Bz': 0.748499,
    'sensor_8_Bx': 0.669811, 'sensor_8_By': 0.638606, 'sensor_8_Bz': 0.765531,
}

# 提取标准差的值并计算方差（方差 = std^2）
measurement_noise = np.array([std**2 for std in std_values.values()])
print(measurement_noise)
initial_estimate = 0  # 初始状态估计
initial_covariance = 1  # 初始估计误差协方差
kf_dict = {}
for i in range(24):  # 对于每个轴（x, y, z）
    kf_dict[i] = KalmanFilter(process_noise, measurement_noise[i], initial_estimate, initial_covariance)
...
def calculation_parallel(magcount=1, use_kf=0, use_wrist=False):
    ...
    global initial_covariance
    global process_noise
    global measurement_noise
    global kf_dict
    # global direction
    myparams1 = params
    myparams2 = params2
    while True:
        if not worklist.empty():
               ...
               # 创建一个字典，存储每个轴的卡尔曼滤波器实例
                ...
                # 存储每个传感器轴的数据去噪结果
                filtered_data = np.zeros_like(datai)

                # 对每个传感器的每个轴进行卡尔曼滤波处理
                for i in range(datai.shape[0]):  # 对于每个传感器
                    for j in range(datai.shape[1]):  # 对于每个轴（x, y, z）
                        filtered_data[i, j] = kf_dict[i*3+j].update(datai[i, j])
                ...
                mapping_net_model.load_state_dict(torch.load('/home/czy/桌面/magx-main1/Codes/optimization/1226mapping_net_model.pth',map_location=torch.device('cpu')))

                mlp_model = MLP().to(device)
                mlp_model.load_state_dict(torch.load('/home/czy/桌面/magx-main1/Codes/optimization/1215_differential_mlp_model_v1.pth',map_location=torch.device('cpu')))
            ...
            #     model.eval()
                ...
                if(len(resultslist)==300):
                     ...
                     test.to_csv("/home/czy/windows_disk/Users/26911/Documents/linux/trackingdata/1228MLP4_"+str(countnum)+".csv")
                     print("Exited")
                     countnum=countnum+1
                     resultslist=[]
                # current = [datetime.datetime.now()]
