...
params = np.array([np.log(0.46), 1e-2 * (0), 1e-2 * (-7.5), 1e-2 * (0), 0, 0])
...
def calculation_parallel(magcount=1, use_kf=0, use_wrist=False):
    ...
    data = pd.read_csv("/home/czy/桌面/magx-main1/Codes/optimization/alltest_LM_filtered.csv")
    ...
    # calibration = np.load('/home/czy/桌面/MetaJoint-master/Code/tracking_method/result/calibration2.npz')
    ...
    for j in range (0,len(data)):
        ...
        # for i in range(0,24):
            # 遍历24个传感器数据
        for i in range(8):  # 8个传感器
            # 构造每个传感器的列名
            Bx_column = f'sensor_{i+1}_Bx'
            By_column = f'sensor_{i+1}_By'
            Bz_column = f'sensor_{i+1}_Bz'
            
            # 提取传感器数据
            sensors[i, 0] = data[Bx_column][j]  # 提取 Bx
            sensors[i, 1] = data[By_column][j]  # 提取 By
            sensors[i, 2] = data[Bz_column][j]  # 提取 Bz
    ...
    # test.to_csv("/home/czy/桌面/magx-main1/Codes/optimization/LMDiff_predictions.csv")
    print("Exited")
