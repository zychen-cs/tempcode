...
#                 #    np.pi/2, np.pi
# Position: -0.05, -3.32, -0.19, m=-1.39, dis=3.32,orientation: 0.00,4.31
params = np.array([-0.05, -3.32, -0.19, 0.05, 4.31])
                #    np.pi/2, np.pi
# Change this parameter for different initial value for 2 magnets
params2 = np.array([
    40 / np.sqrt(2) * 1e-6, 40 / np.sqrt(2) * 1e-6, 0, np.log(0.5),
    1e-2 * 2.6, 1e-2 * 6.8, 1e-2 * (-4.8), 0, 0,
    1e-2 * 3.5, 1e-2 * (6.4), 1e-2 * (4), 0, 0,
])
countnum=1
countnum1=1
resultslist=[]
    ...
    while True:
        if not worklist.empty():
            ...
            if magcount == 1:
                ...
                #     myparams1 = params
                resulti = cs.ekf1mag(datai.reshape(-1), pSensor.reshape(-1), myparams1)
                ...
                #      test.to_csv("/home/czy/windows_disk/Users/26911/Documents/linux/trackingdata/0112test1_"+str(countnum)+".csv")
                #      print("Exited")
                #      countnum=countnum+1
                #      resultslist=[]
                # resultlist.append(current)
                # print(resulti[3])
                # print("Orientation:":resulti[3],resulti[4])
                print("Position: {:.2f}, {:.2f}, {:.2f}, dis={:.2f},orientation: {:.2f},{:.2f}".format(
                    resulti[0],
                    resulti[1],
                    result[2],
                    np.sqrt(
                        result[0] ** 2 + result[1] ** 2 + result[2] ** 2),
                    ang_convert1(resulti[3]),
                    ang_convert(resulti[4]))),
...
def notification_handler(sender, data):
    ...
    if(len(resultslist1)==1000):
        ...
        test.to_csv("/home/czy/windows_disk/Users/26911/Documents/linux/trackingdata/0114sensor_"+str(countnum1)+".csv")
        print("Exited")
