...
params = np.array([np.log(0.46), 1e-2 * (0), 1e-2 * (-6), 1e-2 * (0.57), 0, 0])
        ...
        if not worklist.empty():
            ...
            #     model.eval()
                ...
                with torch.no_grad():
                    ...
                    X_denoised_np = reconstruc_tensor.cpu().numpy().transpose(0, 2, 1).reshape(1, -1)  # shape: (464, 84)
                    print("Input to tracking model shape:", X_denoised_np.shape)
                    readings = X_denoised_np
                  
                    if magcount == 1:
                        if np.max(np.abs(myparams1[1:4])) > 0.5:
                            myparams1 = params
                        resulti = cs.solve_1mag(
                            readings, pSensor.reshape(-1), myparams1)
                        myparams1 = resulti
                        endtime = time.time() 
                        ...
                        if(len(resultslist)==150):
                            print("Output csv")
                            test = pd.DataFrame(columns=name,data=resultslist)
                            test.to_csv("/home/czy/windows_disk/Users/26911/Documents/linux/trackingdata/0707_addpoint_motion2_"+str(countnum)+".csv")
                            print("Exited")
                            countnum=countnum+1
                            resultslist=[]
                        # print(resultslist)
                ...
                #      print("Output csv")
                #      test = pd.DataFrame(columns=name,data=resultslist)
                #     #  test.to_csv("/home/czy/windows_disk/Users/26911/Documents/linux/trackingdata/0613data_ours1_"+str(countnum)+".csv")
                #      print("Exited")
                #      countnum=countnum+1
                #      resultslist=[]
                # # current = [datetime.datetime.now()]
