...
# ble_address = "CE:A2:4C:B9:20:E2"

ble_address = "E3:B2:FC:BD:2B:1E"
...
cali_path = '/home/czy/桌面/magx-main1/0621cali.csv'
# cali_path = '/home/czy/桌面/magx-main1/0526cali.csv'
name = ['Time Stamp', 'x',
        'y', 'z', 'runtime']
...
def calculation_parallel(magcount=1, use_kf=0, use_wrist=False):
    ...
    # tracking_model.load_state_dict(torch.load('/home/czy/桌面/magx-main1/Codes/optimization/0130_differential_mlp_model_v3.pth',map_location=torch.device('cpu')))
    # tracking_model.load_state_dict(torch.load('/home/czy/桌面/magx-main1/Codes/optimization/0526_differential_mlp_model_v6.pth',map_location=torch.device('cpu')))
    ...
    while True:
        if not worklist.empty():
            ...
            #     model.eval()
                ...
                # 使用模型进行预测
                with torch.no_grad():
                    y_pred_tensor = tracking_model(datai_tensor)
                ...
                if(len(resultslist)==300):
                     ...
                     test.to_csv("/home/czy/windows_disk/Users/26911/Documents/linux/trackingdata/0621env2_Z_"+str(countnum)+".csv")
                    #  test.to_csv("/home/czy/windows_disk/Users/26911/Documents/linux/trackingdata/0605test_1_"+str(countnum)+".csv")
                     print("Exited")
                     countnum=countnum+1
                     resultslist=[]
                # current = [datetime.datetime.now()]
