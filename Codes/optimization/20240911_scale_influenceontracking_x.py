...
def magnetic_moment(m, theta, phi):
    # 将 m 从球坐标系转换到直角坐标系
    mx = m * np.sin(theta) * np.cos(phi)  # X 轴分量
    my = m * np.sin(theta) * np.sin(phi)  # Y 轴分量
    mz = m * np.cos(theta)               # Z 轴分量
...
def calculation_parallel():
    ...
    dis = 3.04  # 计算 scale_factor=0 时的距离
    ...
    for j in range(len(data)):
        ...
        # 计算磁场强度并存储结果
        result_draw_0.append(sensors[18]-3)
    ...
    for scale_factor in scale_factors:
        ...
        for j in range(len(data)):
            ...
            # 计算磁场强度并存储结果
            result_draw.append(sensors[19]+24)
    ...
    plt.ylabel('Estimated Distance (m)', fontsize=20)
